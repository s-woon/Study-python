...
def Pb_read():
      try:
            with open('phonebook.txt', 'r') as f:
                  ...
                  n = 3
            person = [data[i * n:(i + 1) * n] for i in range((len(data) + n - 1) // n)]
            return person
...
def member_print():
      ...
      for name, age, address in persons:
            print('{0:<5} │ {1:>3} │ {2:<20}'.format(name, age, address))
      print('\n  * 주소록 출력완료 *\n')
...
def member_read():
      ...
      print('{0:^7}  {1:^2}  {2:^20}'.format('이름', '나이', '주소'))
      for i in range(len(persons)):
            print('{0:<5} │ {1:>3} │ {2:<20}'.format(persons[i][0], persons[i][1], persons[i][2]))
      print()
